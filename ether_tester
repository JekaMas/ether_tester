...
class Cluster(object):
    def __init__(self, containers, is_wait_sync=False):
        self.is_wait_sync = is_wait_sync
...
class StatisticsCollector(object):
    ...
    def get_stats(self, n, test_scenario=None):
        base_levels = []
        stats_list = []
        containers_count = len(self.containers)

        for j in range(0, containers_count):
            base_level_str = StringIO(self.containers[j].get_net_stats())
        ...
        if test_scenario is not None:
            for i in range(0, len(test_scenario)):
                template_dict['$step{num}'.format(num=i)] = ''

        while delta < n:
            # todo: run in parallel
            for i in range(0, containers_count):
                if test_scenario is not None:
                    # in test_scenario the results of prev steps can be used in placeholders like '$step0' -
                    ...
                    for j, test_command in enumerate(test_scenario):
                        ...
                        template_dict['$step{num}'.format(num=j)] = res
        ...
        for j in range(0, containers_count):
            stats_list[j] = stats_list[j].astype(int)
...
cluster = Cluster([g1, g2], is_wait_sync=True)
cluster.start()

geth1 = ContainerManager(name="geth_with_whisper", container=g1)
geth2 = ContainerManager(name="geth_without_whisper", container=g2)

stats = StatisticsCollector([geth1, geth2], debug=True)
stats_list = stats.get_stats(60)

print(g1.description, g1.command)
print("Stats (bytes/s):\n", stats_list[0].describe(include='all').astype(int))
print("\nTraffic sum(bytes):\n", stats_list[0].sum(axis=0))
print()

print(g2.description)
...
simple
stats_list = stats.get_stats(60, ['personal.newAccount(\"passphrase\")'])

with template (https://docs.python.org/3.1/library/string.html#template-strings)
stats_list = stats.get_stats(60, ['personal.newAccount(\"passphrase\")', 'miner.setEtherbase($step0)'])
'''
